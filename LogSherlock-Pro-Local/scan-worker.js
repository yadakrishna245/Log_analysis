...
self.onmessage = async function(e) {
    ...
    function classifyFile(name, size) {
        ...
        // ═══ HPSReport Windows files — skip noise, keep useful ═══
        const bname = n.split('/').pop();
        const ext2 = '.' + bname.split('.').pop();
        // Binary/useless Windows formats
        if (['.evtx','.dmp','.hiv','.nfo','.cab'].includes(ext2)) return 'skip';
        // Windows noise log files (partial match handles prefixed names like LD7-ANTD-V001_Dism_LOG.TXT)
        if (/setupapi.*log|dism|sfc.*cbs.*log|panther.*setup.*log/i.test(bname)) return 'skip';
        // File/process listing exports (no diagnostic value)
        if (/_dll\.(csv|txt)$|_exe\.(csv|txt)$|_drivers\.(csv|txt)$/.test(bname)) return 'skip';
        if (/process\.(csv|txt)$|pstat\.txt$/i.test(bname)) return 'skip';
        if (/system32_dll|system32_exe|syswow64_/i.test(bname)) return 'skip';
        if (/programfiles.*sys\.(csv|txt)$|programdata.*sys\.(csv|txt)$/i.test(bname)) return 'skip';
        // PowerShell config, powercfg
        if (/powershell_config|powercfg/i.test(bname)) return 'skip';
        // Installed apps/services/QFE lists
        if (/installed.*\.(txt|xml)$|_qfes\.(txt|xml)$|running.*svcs|systemdr/i.test(bname)) return 'skip';
        // Registry dumps, security policy, firewall, GPO
        if (/_reg\.txt$|securitypolicy|firewall.*adv|ipsec\.txt|gpresult/i.test(bname)) return 'skip';
        // Progress/schedule/misc
        if (/progress.*\.txt$|prcmon|schtasks|diskshadow/i.test(bname)) return 'skip';
        // Network info, misc
        if (/winrm|winsat|pnputil|wdsutil|whoami|regionalsettings|mounteddevices/i.test(bname)) return 'skip';
        if (/recovery\.txt|time_w32tm|print\.|fltmc|_misc\.|terminal_server|system_policy|wer_|disktimeout/i.test(bname)) return 'skip';
        if (/bcdedit|driververifier|proxycfg|proxyie|secureboot|fsrm_|deviceguard|trustedplatform|smb\.|snmp_|spool|sql\.|dhcp_logs|debugdir|nvspbind|_toe\./i.test(bname)) return 'skip';
        // CSV files — skip all except event_log CSVs
        if (ext2 === '.csv' && !/event_log/i.test(bname)) return 'skip';

        // ═══ HPSReport Windows files — KEEP these (useful diagnostics) ═══
        if (/event_log.*\.txt$/i.test(bname)) return 'high';
        if (/devcon\.txt|disk_fsutil|disk_information|vssadmin|storage\.txt/i.test(bname)) return 'medium';
    ...
    function scanLines(text, fileName, mtime) {
        ...
        for (let ln = 0; ln < Math.min(totalLineCount, headLimit); ln++) {
            ...
            // Skip CSV-formatted lines (Windows Event Log exports)
            if (line.charAt(0) === '"' && line.indexOf('","') !== -1 && (line.match(/","/g)||[]).length >= 4) continue;
